...
CLASSIFIER_MODEL = os.environ.get("PIER39_LABEL_MODEL", "gpt-4o-mini")
...
class ClassifierPolicy:
    ...
    def _load(self, store: StoreConfig) -> dict[str, str]:
        ...
        if store.slug not in self._cache:
            ...
            if path.exists():
                self._cache[store.slug] = json.loads(path.read_text(encoding="utf-8"))
            else:
                self._cache[store.slug] = {}
    ...
    def flush(self, store: StoreConfig) -> None:
        ...
        path.write_text(
            json.dumps(self._cache[store.slug], indent=2, sort_keys=True, ensure_ascii=False),
            encoding="utf-8",
        )
