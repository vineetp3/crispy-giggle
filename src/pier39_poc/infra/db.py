...
Written by ingest.merge and ingest.indexing; read by retrieval and evaluation.
...
from pier39_poc.infra.config import REPO_ROOT, database_url
...
def upsert_product(conn: psycopg.Connection[DictRow], store_id: int, product: Product) -> int:
    row = conn.execute(
        """
        ...
        (
            ...
            [c.get("handle") for c in product.collections if c.get("handle")],
            product.updated_at,
        ),
    ).fetchone()
    assert row is not None
