...
FIXTURES = Path(__file__).parent.parent / "fixtures" / "skout"
...
def test_commerce_facts_are_rejected_but_ratings_are_not():
    ...
    """
    from pier39_poc.core.quotability import is_commerce_fact
...
def test_abandoned_skus_are_not_selectable():
    ...
    from pier39_poc.ingest.crawl import selectable

    products = [
        Product(handle="live", online_store_url="https://x/products/live", sellable=True),
        Product(handle="dead", online_store_url="https://x/products/dead", sellable=False),
        Product(handle="unpublished", online_store_url=None, sellable=True),
        Product(handle="legacy", online_store_url="https://x/products/legacy"),
    ]
